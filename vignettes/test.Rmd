...
reference.site <- "Toronto"
reference.time <- "baseline"

```

**Tip** Cases in which instruments are cross-calibrated at a single time point, the reference time point does not need to be specified. 
...
diagnostic.plot(co, which.parameter = "Tb. vBMD")
```
...
```{r, fig.width= 7, fig.height = 4}
diagnostic.plot(co, which.parameter = "Tb. vBMD", highlight.site = "Odense")
```
...
# retrieve stored results (as datatables)
# mvp.results.calibrated <-get.results(object = co, 
#                                 which.results = "mvp.analysis",
#                                 which.data = "all",
#                                 format = 'dt') 
